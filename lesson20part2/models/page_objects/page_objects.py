...
from lesson20part2.models.locator import AdminPageLocators, DashboardLocators
from lesson20part2.models.locator import MainPageLocators, ProductPageLocators
from lesson20part2.models.page import BasePage
...
class DashboardPage(BasePage):
    ...
    def get_catalog_menu(self, delay):
        try:
            WebDriverWait(self.driver, delay).\
                until(EC.presence_of_element_located(DashboardLocators.CATALOG_MENU))
            return self.driver.find_element(*DashboardLocators.CATALOG_MENU)
        except NoSuchElementException:
            raise AssertionError("Element not founded!")

    def get_marketing_menu(self, delay):
        try:
            WebDriverWait(self.driver, delay).\
                until(EC.presence_of_element_located(DashboardLocators.MARKETING_MENU))
            return self.driver.find_element(*DashboardLocators.MARKETING_MENU)
        except NoSuchElementException:
            raise AssertionError("Element not founded!")
    ...
    def get_product(self, product_name, delay):
        try:
            ...
            products = self.get_table_items(delay)
    ...
    def get_coupon(self, coupon_code, delay):
        try:
            WebDriverWait(self.driver, delay).until(EC.presence_of_element_located(ProductPageLocators.TABLE_BODY))
            products = self.get_table_items(delay)
            for item in products:
                WebDriverWait(item, delay).until(EC.presence_of_element_located(ProductPageLocators.ROW_COLUMN))
                code = item.find_elements(*ProductPageLocators.ROW_COLUMN)[2]
                if code.text == coupon_code:
                    return item
            return False
        except (NoSuchElementException, TimeoutException):
            return False

    def get_table_items(self, delay):
        try:
            WebDriverWait(self.driver, delay).until(EC.presence_of_element_located(ProductPageLocators.PRODUCT_TABLE))
            table = self.driver.find_element(*ProductPageLocators.PRODUCT_TABLE)
            tbody = table.find_element(*ProductPageLocators.TABLE_BODY)
    ...
    def get_download_file_list(self):
        try:
            table = self.driver.find_element(*DashboardLocators.FILE_FORM)
            table_body = table.find_element(*DashboardLocators.TABLE_BODY)
            rows = table_body.find_elements(*DashboardLocators.TABLE_ROWS)
            return rows
        except NoSuchElementException:
            raise AssertionError("Element not founded!")
